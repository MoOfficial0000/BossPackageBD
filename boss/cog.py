...
DAMAGERNG = [
    0,
    2000,
]  # Damage a boss can deal IF attack_amount has NOT been inputted in /boss admin attack.
# Min Damage, Max Damage


class JoinBossButton(discord.ui.Button["BossJoinLayout"]):
    """Join button for boss battles."""

    def __init__(self, boss_cog: "Boss"):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if interaction.user.id in self.boss_cog.disqualified:
            return await interaction.followup.send(
                "You have been disqualified", ephemeral=True
            )
        ...
        await self.boss_cog._log_action(
            f"{interaction.user} has joined the {self.boss_cog.bossball} Boss Battle."
        )


class BossLayout(discord.ui.LayoutView):
    """Shared Components v2 layout for boss battle messages."""
    ...
    def __init__(
        ...
        *,
        ...
        action_row: discord.ui.ActionRow[Any] | None = None,
        timeout: float | None = None,
    ) -> None:
        super().__init__(timeout=timeout)

        for index, section in enumerate(sections):
            self.add_item(discord.ui.TextDisplay(section))
            if index < len(sections) - 1 or files or action_row:
        ...
        if files:
            self.add_item(
                discord.ui.MediaGallery(
                    *(
                        discord.MediaGalleryItem(media=f"attachment://{file.filename}")
                        for file in files
                    )
                )
            )
...
class BossJoinLayout(BossLayout):
    """Join layout for a newly-started boss battle."""
    ...
    def __init__(
        ...
        *,
        boss_name: str,
        boss_hp: int,
        boss_file: discord.File,
    ) -> None:
        action_row = discord.ui.ActionRow()
        action_row.add_item(JoinBossButton(boss_cog))
        super().__init__(
            [
                f"# The boss battle has begun! {boss_cog.bot.get_emoji(boss_cog.bossball.emoji_id)}",
                f"-# Boss: {boss_name}",
                f"-# HP: {boss_hp}",
            ],
            files=[boss_file],
...
class BossBattle(models.Model):
    ...
    attack_power = models.IntegerField(
        default=500, help_text="Attack power of the boss"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(
        default=True, help_text="Whether the boss battle is active"
    )
...
class BossBattleParticipant(models.Model):
    ...
    total_damage_dealt = models.IntegerField(
        default=0, help_text="Total damage dealt by this participant"
    )
...
class BossBattleRound(models.Model):
    ...
    is_attack_phase = models.BooleanField(
        default=True, help_text="Whether this is attack phase"
    )
    boss_damage_taken = models.IntegerField(
        default=0, help_text="Total damage taken this round"
    )
...
class Boss(commands.GroupCog, name="boss"):
    ...
    def admin_permissions_check():
        ...
        async def check(interaction: discord.Interaction["BallsDexBot"]) -> bool:
            ...
            try:
                ...
                dj_user = await user_model.objects.filter(
    ...
    async def start(
        ...
        if self.boss_enabled:
            await interaction.followup.send(
                "A boss battle is already active!", ephemeral=True
            )
            return
        ...
        try:
            ...
            await interaction.followup.send(
                f"Boss battle started with {ball.country}!", ephemeral=True
            )
            ...
            await interaction.followup.send(
                f"Error starting boss battle: {e}", ephemeral=True
            )
    ...
    async def select(self, interaction: Interaction, ball: BallInstanceTransform):
        ...
        if not self.picking:
            return await interaction.followup.send(
                "It is not yet time to select a ball", ephemeral=True
            )
        ...
        if ball in self.balls:
            return await interaction.followup.send(
                "You cannot select the same ball twice", ephemeral=True
            )
    ...
    async def ongoing(self, interaction: Interaction):
        ...
        if user_damage == 0:
            if interaction.user.id in self.users:
                await interaction.followup.send(
                    "You have not dealt any damage.", ephemeral=True
                )
                ...
                await interaction.followup.send(
                    "You have not joined the battle, or you have died.", ephemeral=True
                )
        else:
            if interaction.user.id in self.users:
                await interaction.followup.send(
                    f"You have dealt {user_damage} total damage.\n\n{damage_details}",
                    ephemeral=True,
                )
            elif interaction.user.id in self.disqualified:
                await interaction.followup.send(
                    f"You have dealt {user_damage} damage and have been disqualified.\n\n{damage_details}",
                    ephemeral=True,
                )
            else:
                await interaction.followup.send(
                    f"You have dealt {user_damage} damage and you are now dead.\n\n{damage_details}",
                    ephemeral=True,
                )
    ...
    async def conclude(
        self, interaction: discord.Interaction["BallsDexBot"], winner: str
    ):
        """
        ...
        if self.lasthitter not in self.users and winner == "LAST":
            return await interaction.followup.send(
                "The last hitter is dead or disqualified.", ephemeral=True
            )
        ...
        stats_file = discord.File(
            fp=io.StringIO(stats_content), filename="totalstats.txt"
        )
    ...
    async def endround(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        if not self.picking:
            return await interaction.followup.send(
                "There are no ongoing rounds, use `/boss admin_attack` or `/boss admin_defend` to start one",
                ephemeral=True,
            )
        ...
        for user_id in snapshotusers:
            if [user_id, self.round] not in self.usersinround:
                ...
                if str(user) not in self.currentvalue:
                    self.currentvalue += (
                        str(user) + " has not selected on time and died!\n"
                    )
        ...
        if not self.attack:  # Boss was defending
            if int(self.bossHP) <= 0:
                await self._send_layout_message(
                    interaction.channel,
                    [
                        f"# Round {self.round} has ended {self._boss_emoji()}",
                ...
                await self._send_layout_message(
                    ...
                    [
                        f"# Round {self.round} has ended {self._boss_emoji()}",
                        f"There is {self.bossHP} HP remaining on the boss",
                    ],
                )
        ...
        if self.currentvalue:
            ...
            stats_file = discord.File(
                fp=io.StringIO(stats_content), filename="roundstats.txt"
            )
            await interaction.channel.send(file=stats_file)
    ...
    async def attack(
        ...
        if self.picking:
            return await interaction.followup.send(
                "There is already an ongoing round", ephemeral=True
            )
        if len(self.users) == 0:
            return await interaction.followup.send(
                "There are not enough users to start the round", ephemeral=True
            )
        if self.bossHP <= 0:
        ...
        await self._send_layout_message(
            ...
            [
                f"Round {self.round}",
                f"# {self.bossball.country} is preparing to attack! {self._boss_emoji()}",
                "> Use `/boss select` to select your defending ball.",
        ...
        self.bossattack = (
            ...
            else random.randint(DAMAGERNG[0], DAMAGERNG[1])
        )
    ...
    async def defend(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        await self._send_layout_message(
            ...
            [
                f"Round {self.round}",
                f"# {self.bossball.country} is preparing to defend! {self._boss_emoji()}",
                "> Use `/boss select` to select your attacking ball.",
    ...
    async def disqualify(
        ...
        if resolved_user_id in self.disqualified:
            if undisqualify:
                self.disqualified.remove(resolved_user_id)
                await interaction.followup.send(
    ...
    async def hackjoin(
        ...
        if not self.boss_enabled:
            return await interaction.followup.send("Boss is disabled", ephemeral=True)
        if resolved_user_id in self.users:
            return await interaction.followup.send(
                "This user is already in the boss battle.", ephemeral=True
            )

        self.users.append(resolved_user_id)
        if resolved_user_id in self.disqualified:
            self.disqualified.remove(resolved_user_id)

        await interaction.followup.send(
            f"{user} has been force-joined into the Boss Battle.", ephemeral=True
        )
        await self._log_action(
    ...
    async def ping(
        ...
        if len(self.users) == 0:
            return await interaction.followup.send(
                "There are no users joined/remaining", ephemeral=True
            )
        ...
        if pingsmsg == "":
            ...
            await interaction.followup.send(
                "Message too long, exceeds 2000 character limit", ephemeral=True
            )
...
Damage Records: {len(self.usersdamage)}"""
    ...
    async def _reward_winner(self, bosswinner: int, channel=None):
        ...
        try:
            ...
            if boss_special:
                ...
                await self._log_action(
                    f"`BOSS REWARDS` gave {settings.collectible_name} {self.bossball} to {bosswinner_user}. "
                    f"Special=Boss"
                    f"ATK=0 HP=0"
                )
                ...
                if channel:
                    await self._send_layout_message(
                        ...
                        [
                            ...
                            f"<@{bosswinner}> has won the Boss Battle!",
    ...
    def _boss_emoji(self) -> str:
        if self.bossball is None:
            return ""
        return str(self.bot.get_emoji(self.bossball.emoji_id))

    async def _send_layout_message(
        self,
        ...
        *,
        ...
        await channel.send(
            view=BossLayout(sections, files=files),
            files=files or [],
        )
